...
def load_accounts_from_sheet(sheet_url: str) -> pd.DataFrame:
    ...
    if not client:
        st.warning("Google Sheets not connected — using demo data. See sidebar to connect.")
        return None
    try:
        ...
        for col in ["spend","roas","cpl","leads","ctr","cpc","frequency","engagements","impressions"]:
...
with st.sidebar:
    ...
    sheet_url = st.text_input(
        ...
        help="Share your sheet with the service account email, then paste the URL here."
    )
    if sheet_url:
        if st.button("🔄 Sync from Sheet", use_container_width=True):
            ...
            if df_loaded is not None:
                ...
                st.success(f"Loaded {len(df_loaded)} accounts!")
    st.divider()
    st.markdown("**Sheet column names:**")
    st.code("name, industry, spend, roas\ncpl, leads, ctr, cpc\nfrequency, engagements\nimpressions, status", language="text")
    st.divider()
